...
description: Repository-specific workflow for bumping the tossinvest-extensions release version. Use when the user asks to bump, release, prepare, or validate this repo's package version, including patch/minor/major changes and release tag checks.
---
...
8. Pushing `vX.Y.Z` starts the `Release` GitHub Actions workflow. The workflow
   ...
   non-draft or prerelease release intentionally. Set `deploy_docs=true` on a
   published manual run only when GitHub Pages is enabled for this repository.
