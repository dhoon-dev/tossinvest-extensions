...
def _read_versions() -> dict[str, str]:
    return {
        ...
        "uv.lock package tossinvest-extensions": _read_lock_version(),
        "src/tossinvest_extensions/_version.py __version__": _read_literal_assignment(
            ROOT / "src" / "tossinvest_extensions" / "_version.py", "__version__"
        ),
...
def _read_lock_version() -> str:
    ...
    for package in packages:
        if not isinstance(package, dict) or package.get("name") != "tossinvest-extensions":
            continue
        version = package.get("version")
        if isinstance(version, str):
            return version
        msg = "Missing string version for tossinvest-extensions in uv.lock."
        raise TypeError(msg)
    msg = "Missing tossinvest-extensions package in uv.lock."
